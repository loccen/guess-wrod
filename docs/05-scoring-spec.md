...
V0.2 起，默认回灌答案上下文，并把本局历史猜词通过多轮消息回放给模型，避免继续放大已经验证过的宽泛错误方向。
...
8. 提示词组装按“同一局游戏的多轮对话”组织：固定背景作为稳定前缀，历史猜词按 `user -> assistant` 轮次回放，当前猜词作为新的 `user` 轮次追加。
9. AI Gateway 日志、缓存和成本观测只作为基础设施能力，不替代业务侧 D1 缓存语义。

### 10.2.1 多轮组装与缓存命中

参考 DeepSeek 官方文档里的“多轮对话”和“上下文硬盘缓存”：

1. `/chat/completions` 是无状态接口，服务端不会替你记住对话历史，所以每次请求都要自行拼接当前游戏此前的全部轮次。
2. DeepSeek 的硬盘缓存只匹配“完整前缀单元”，因此越稳定、越前置、越少变动的前缀越容易命中。
3. V0.2 的组装方式分三层：
   1. `system`：长规则提示词，跨游戏尽量稳定。
   2. 第二个 `system`：当前这局游戏的固定背景，只包含 `answer`、`answer_context`、`language`、`scoring_rules_version`、`relation_caps`。
   3. 历史轮次：每个历史猜词都回放成一组 `user(仅猜词词面)` + `assistant(该轮评分 JSON)`；当前猜词再作为新的 `user(仅猜词词面)` 追加到末尾。
4. 这样从第二轮开始，后续请求可以稳定复用“系统提示词 + 游戏固定背景 + 已完成轮次”的完整前缀，更符合 DeepSeek 的缓存命中规则。
5. 不再额外发送 `guess_history` 汇总字段，避免同一份历史既出现在结构化字段里、又重复出现在多轮消息里。
6. 不同游戏绝不能共用轮次历史；组装时只能读取当前 `game_id` 下的猜词记录。

### 10.2.2 系统提示词约束重点

V0.2 的系统提示词控制在较紧凑的长度内，但仍显式强调以下规则：
...
5. 模型必须结合此前多轮记录做动态纠偏：如果历史里已经出现多个宽泛高分方向，后续同方向词通常不应继续升高。
6. `reason` 必须明确指出高分是因为核心类别/功能/部件/具体场景，还是低分是因为词太宽泛、太抽象、太像重复误导方向。
7. 提示词本身不追求面面俱到，而是优先保留最影响误导性的硬约束，避免 system 段过长稀释缓存收益。
