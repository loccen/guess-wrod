...
3. V0.2 的组装方式分三层：
   ...
   2. 首个 `user`：当前这局游戏的固定背景，只包含 `answer`、`answer_context`、`language`、`scoring_rules_version`、`relation_caps`。
   3. 历史轮次：每个历史猜词都回放成一组 `user(第 N 轮猜词)` + `assistant(该轮评分 JSON)`；当前猜词再作为新的 `user` 轮次追加到末尾。
4. 这样从第二轮开始，后续请求可以稳定复用“系统提示词 + 游戏固定背景 + 已完成轮次”的完整前缀，更符合 DeepSeek 的缓存命中规则。
5. 不同游戏绝不能共用轮次历史；组装时只能读取当前 `game_id` 下的猜词记录。
