...
| T27 | 行为分析聚合 | P1 | T22 | 是 | 漏斗、留存、复玩统计 | 可产出每日行为指标 |
| T28 | 评分质量分析 | P1 | T14,T24,T26 | 是 | 质量排行和问题样本 | 可定位高反馈答案和异常评分 |
| T29 | 成本分析报表 | P1 | T23 | 是 | 成本日报 | 可计算单局成本和缓存收益 |
| T30 | 后端接口测试 | P0 | T03,T05,T14,T15,T16 | 是 | API 测试 | 主流程和异常通过 |
| T31 | 前端主流程测试 | P0 | T19,T20,T21 | 是 | 浏览器测试 | 手机视口可完整游玩 |
| T32 | 部署配置 | P0 | T01,T30,T31 | 否 | H5 和 API 部署配置 | 公网可访问 |
| T33 | 上线前检查 | P0 | T22,T23,T27,T28,T29,T30,T31,T32 | 否 | 检查记录 | HTTPS、密钥、答案安全、限流、分析日报均通过 |
